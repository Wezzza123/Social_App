...
import { RoleEnum } from "../DataBase/models/user.model"; // ✅ خليها DataBase زي عندك

export const authenticationMiddleware = (
  tokenType: TokenTypeEnum = TokenTypeEnum.accses // ✅ accses زي ما معرفاه عندك
) => {
...
export const authorizationMiddleware = (accessRoles: RoleEnum[]) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { decoded, user } = await tokenService.decodeToken({
      authorization: req.headers.authorization,
      tokenType: TokenTypeEnum.accses, // ✅ accses برضه
    });
